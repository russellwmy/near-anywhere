        ...
        std::str::from_utf8(data)
            ...
            .map_err(|_| {
                std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!(
                        "raw key does not have a valid AccountId to be TrieKey::{}",
                        trie_key
                    ),
                )
            })
    ...
    fn parse_account_id_from_trie_key_with_separator(
        ...
        if let Some(account_id) = next_token(account_id_prefix, ACCOUNT_DATA_SEPARATOR) {
            ...
            Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!(
                    "raw key does not have ACCOUNT_DATA_SEPARATOR to be TrieKey::{}",
                    col_name
                ),
            ))
        }
...
#[cfg(test)]
mod tests {
    use {super::*, near_anywhere_crypto::KeyType};
